...
#define DISK_STATUS_READY                                                      \
  (1 << 6) // 控制器就绪(硬盘控制器未就绪的情况下不能向它发送请求)
#define DISK_STATUS_SEEK (1 << 4)  // 正在寻道
#define DISK_STATUS_REQ (1 << 3)   // 数据请求(该位为1才能进行磁盘读写)
#define DISK_STATUS_ERROR (1 << 0) // 命令执行错误

#define wait_when_disk0_busy()                                                 \
  while (in8(PORT_DISK0_STATUS) & DISK_STATUS_BUSY)                            \
  nop()
#define wait_when_disk0_not_ready()                                            \
  while (!(in8(PORT_DISK0_DATA) & DISK_STATUS_READY))                          \
  nop()
#define wait_when_disk0_not_req()                                              \
  while (!(in8(PORT_DISK0_STATUS) & DISK_STATUS_REQ))                          \
  nop()

#define wait_when_disk1_busy()                                                 \
  while (in8(PORT_DISK1_STATUS) & DISK_STATUS_BUSY)                            \
  nop()
#define wait_when_disk1_not_ready()                                            \
  while (!(in8(PORT_DISK1_DATA) & DISK_STATUS_READY))                          \
  nop()
#define wait_when_disk1_not_req()                                              \
  while (!(in8(PORT_DISK1_STATUS) & DISK_STATUS_REQ))                          \
  nop()
...
generate_disk_work_mode(char access_mode, char hard_disk, char LBA_27_24) {
  return (0b10100000 | (access_mode << 6) | (hard_disk << 4) |
          (LBA_27_24 & 0xf));
}
...
struct block_buffer_node {
  unsigned int
      count; // 当前硬盘访问几个扇区(如果操作多个扇区,每准备好一个扇区就会发出一个中断)
...
struct block_device_operation {
  ...
  long (*transfer)(long cmd, unsigned long blocks, long count,
...
struct Disk_Identify_Info {
  ...
  unsigned short
      addressable_logical_sector_for_28[2]; // 用户可寻址的全部逻辑扇区数
  ...
  unsigned short
      current_APM_level_value; // 当前APM级别(Advanced Power Management)
  ...
  unsigned short
      addressable_logical_sector_for_48[4]; // 用户可寻址的全部逻辑扇区数(48)
...
long IDE_transfer(long cmd, unsigned long blocks, long count,
                  unsigned char *buffer);
long cmd_out();
struct block_buffer_node *make_request(long cmd, unsigned long blocks,
...
void other_handler(long unsigned int, long unsigned int);

/*  添加硬盘操作请求到队列  */
static __attribute__((always_inline)) void
add_request(struct block_buffer_node *node) {
  list_add_to_behind(&disk_request.wait_queue_list.wait_list,
...
/*  LBA28模式  */
static __attribute__((always_inline)) void
Device_mode_LBA28(unsigned int count, unsigned long LBA) {
...
/*  LBA48模式  */
static __attribute__((always_inline)) void
Device_mode_LBA48(unsigned int count, unsigned long LBA) {
...
long IDE_read( unsigned long blocks, long count, unsigned char *buffer);
long IDE_write( unsigned long blocks, long count, unsigned char *buffer);
