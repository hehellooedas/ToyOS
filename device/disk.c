...
hw_int_controller disk_int_controller = {.enable = IOAPIC_enbale,
...
/*  这是磁盘驱动对外提供的接口(API)  */
struct block_device_operation IDE_device_operation = {.open = IDE_open,
                                                      .close = IDE_close,
                                                      .ioctl = IDE_ioctl,
...
void disk_init(void) {
  ...
  register_irq(0x2f, &entry, &disk_handler, (unsigned long)&disk_request,
               &disk_int_controller, "disk1");
...
void disk_handler(unsigned long nr, unsigned long parameter,
  ...
  struct block_buffer_node *node =
      ((struct request_queue *)parameter)->is_using;
  node->end_handler(nr, parameter);
}

void disk_exit() { unregister_irq(0x2f); }
...
{
    return IDE_transfer(ATA_READ,blocks, count, buffer);
}

long IDE_write( unsigned long blocks, long count, unsigned char *buffer)
{
    return IDE_transfer(ATA_WRITE,blocks, count, buffer);
}
...
long cmd_out() {
  wait_queue_T *wait_queue_tmp =
      container_of(get_List_next(&disk_request.wait_queue_list.wait_list),
  ...
  case ATA_READ:
    ...
    out8(PORT_DISK1_CMD,
         node->cmd); // 向硬盘发送读请求(硬盘把数据放置到NV
                     // Cache后会发中断过来,有一个准备数据和触发中断的过程)
    break;
  case ATA_WRITE:
    Device_mode_LBA48(node->count, node->LBA);
    wait_when_disk1_busy();
