...
unsigned int DISK1_FAT32_read_FAT_Entry(struct FAT32_sb_info* fsbi,unsigned int fat_entry)
{
    ...
    memset(buf,0,512);
    IDE_device_operation.transfer(ATA_READ,fsbi->FAT1_firstsector + (fat_entry >> 7),1,(unsigned char*)buf);   //fat_entry >> 7是该目录项所在扇区索引
...
unsigned long DISK1_FAT32_write_FAT_Entry(struct FAT32_sb_info* fsbi,unsigned int fat_entry,unsigned int value)
{
    ...
    memset(buf,0,512);
    IDE_device_operation.transfer(ATA_READ,fsbi->FAT1_firstsector + (fat_entry >> 7),1,(unsigned char*)buf);
    /*  先清空表项原来的值再写入新值  */
    buf[fat_entry & 0x7f] = (buf[fat_entry & 0x7f] & 0xf0000000) | (value & 0xffffffff);

    /*  更新两个FAT表  */
    IDE_device_operation.transfer(ATA_WRITE,fsbi->FAT1_firstsector,0,(unsigned char*)buf);
    IDE_device_operation.transfer(ATA_WRITE,fsbi->FAT1_firstsector,0,(unsigned char*)buf);
...
long FAT32_read(
    ...
    struct FAT32_inode_info* finode = filep->dentry->dir_inode->private_index_info;
 ...
 */
struct dir_entry* FAT32_lookup(
    struct index_node* parent_inode,
...
next_cluster:
    ...
    /*  读取该目录的起始簇号指向的一个簇的数据  */
    if(!IDE_device_operation.transfer(ATA_READ,sector,fsbi->sector_per_cluster,buf))
    {
    ...
    for(int i=0;i<fsbi->bytes_per_cluster;i+=32,tmpdentry++){
        ...
        while(tmpldentry->LDIR_Attr == ATTR_LONG_NAME && tmpldentry->LDIR_Ord != 0xe5){
            for(int x=0;x<5;x++){   //比较Name1
                if(j > destination_dentry->name_length && tmpldentry->LDIR_Name1[x] == 0xffff){
                    continue;
                }else if(j > destination_dentry->name_length || tmpldentry->LDIR_Name1[x] != (unsigned short)destination_dentry->name[j++]){
                    goto continue_cmp_fail;
                }
            }
            for(int x=0;x<6;x++){   //比较Name2
                if(j > destination_dentry->name_length && tmpldentry->LDIR_Name2[x] == 0xffff){
                    continue;
                }else if(j >destination_dentry->name_length || tmpldentry->LDIR_Name2[x] != (unsigned short)destination_dentry->name[j++]){
                    goto continue_cmp_fail;
                }
            }
            for(int x=0;x<2;x++){   //比较Name3
                if(j > destination_dentry->name_length && tmpldentry->LDIR_Name3[x] == 0xffff){
                    continue;
                }else if(j >destination_dentry->name_length || tmpldentry->LDIR_Name3[x] != (unsigned short)destination_dentry->name[j++]){
                    goto continue_cmp_fail;
                }
            }
            ...
            switch (tmpdentry->Dir_Name[x]) {
                case ' ':
                    ...
                    if(!(tmpdentry->Dir_Attr & ATTR_DIRECTORY)){
                        ...
                        }else if(j < destination_dentry->name_length && tmpdentry->Dir_Name[x] == destination_dentry->name[j]){
                            j++;    //文件名里就是有空格
                            break;
                        ...
                        if(j < destination_dentry->name_length && tmpdentry->Dir_Name[x] == destination_dentry->name[j]){
                            j++;
                            break;
                        }else if(j == destination_dentry->name_length){
                            continue;
...
find_lookup_success:    //查找成功
    ...
    finode->first_cluster = (tmpdentry->Dir_FatClusHI << 16 | tmpdentry->Dir_FatClusLO & 0xffffffff);
